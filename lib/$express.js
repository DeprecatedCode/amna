...
module.exports = function (amna, log) {
    ...
    app.use(function (req, res, next) {
        log.request(req, 'init');
        next();
    });

    /**
     * Allow any module to stop all requests
     */
    app.use(function (req, res, next) {
        ...
        if (errs.len) {
            res.json(500, amna.responses.error(errs));
        }
        else {
            next();
        }
    ...
    app.use(function (req, res, next) {
        ...
        if (typeof query === 'string' && query.length && (query[0] === '{' || query[0] === '[')) {
            try {
                ...
                amna.err('Malformed query JSON');
            }

            if (Array.isArray(query)) {
                if (query.length !== 2) {
                    amna.err('JSON query array must contain exactly two elements: [query, options]');
                }
                [0, 1].map(function (idx) {
                    if (!query[idx] || typeof query[idx] !== 'object') {
                        amna.err('\'' + {0: 'query', 1:'options'}[idx] +
                            '\' element in JSON query array [query, options] must be an object');
                    }
                });
        ...
        req.__defineGetter__('requiredJsonQuery', function () {
            amna.err('Missing query JSON object in URL');
        });
